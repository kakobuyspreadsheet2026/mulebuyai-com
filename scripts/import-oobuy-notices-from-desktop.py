...
Examples:
  ...
  python3 scripts/import-oobuy-notices-from-desktop.py --src ~/Desktop/news \\
      --out src/data/officialOopbuyNoticeExcerpts.json --sync-images

The /how-to-buy page uses locale-specific ``howToBuyBuyingGuideBody.{locale}.fragment.html``
(via ``howToBuyBuyingGuide.byLocale.ts``) plus ``public/how-to-buy-guide/`` — not this script’s JSON output.
